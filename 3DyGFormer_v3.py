...
def train(
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch_data in enumerate(train_tqdm):
            ...
            # v3: sync parameters across GPUs after every step so all ranks train the same model
...
# v3: larger default batch_size for multi-GPU
...
parser.add_argument("--lr", type=float, default=1e-4, help="Base learning rate (will be scaled by world_size in multi-GPU)")
...
# v3: linear learning-rate scaling for multi-GPU
effective_lr = args.lr * WORLD_SIZE if IS_MPI else args.lr

global_batch_size = args.batch_size * WORLD_SIZE

log("=" * 80)
log("DyGFormer_v3 Multi-GPU Competition")
