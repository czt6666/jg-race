"""
9DyGFormer_v9 - 修复训练时负样本forward无分块导致 K 无法增大的问题
核心改动（相比 7DyGFormer）：
1. 训练负样本 forward 改为分块（neg_chunk），支持更大 K（如 K=7）而不 OOM
2. seq_len=64（默认，比 7DyGFormer 的 32 更大）
3. K=7 默认（8-way ranking，更接近 100-way test）
其余与 7DyGFormer 相同：InfoNCE loss, PopNegSampler, 100-way val MRR
"""
import os
...
jt.flags.use_cuda = 1
# 限制 Jittor GPU 内存池大小，防止与同 GPU 的其他进程冲突
# device_mem_limit 默认 -1 (无限) → Jittor 会占满整个 GPU，导致 OOM
jt.flags.device_mem_limit = 14 * 1024 * 1024 * 1024  # 14 GB
IS_MPI = jt.in_mpi
...
VAL_NEG_CHUNK = 10  # 每次 forward 处理的负样本数，防止 OOM
...
def train(model, optimizer, train_loader, val_loader, neg_sampler, K,
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch_data in enumerate(pbar):
            ...
            # 负样本分块 forward（防止 B*K 一次性 OOM）
            src_rep = np.repeat(src, K)
            t_rep   = np.repeat(t, K)
            neg_src_embs, neg_embs = [], []
            for c in range(0, B * K, neg_chunk):
                ce = min(c + neg_chunk, B * K)
                se, de = backbone.compute_src_dst_node_temporal_embeddings(
                    src_rep[c:ce], neg_dst[c:ce], t_rep[c:ce])
                neg_src_embs.append(se)
                neg_embs.append(de)
            src_emb_n = jt.concat(neg_src_embs, dim=0)
            neg_emb   = jt.concat(neg_embs, dim=0)
            neg_logit = predictor(src_emb_n, neg_emb).squeeze(-1)    # [B*K]
            del src_emb_n, neg_emb, neg_src_embs, neg_embs
        ...
        if val_loader is not None:
            ...
            if cur > best_mrr:
                ...
                patience_counter = 0
                if IS_MAIN:
                    jt.save(model.state_dict(), f"{save_path}/{dataset_name}_DyGFormerV9_best.pkl")
                    os.sync()
            ...
            if IS_MAIN:
                jt.save(model.state_dict(), f"{save_path}/{dataset_name}_DyGFormerV9.pkl")
                os.sync()
...
parser.add_argument("--max_seq_len", type=int, default=64,
                    help="序列长度，默认64（比7DyGFormer的32更大）")
parser.add_argument("--dropout", type=float, default=0.1)
parser.add_argument("--lr", type=float, default=1e-4)
# 核心参数
parser.add_argument("--num_neg", type=int, default=7,
                    help="InfoNCE 负样本数，默认7（8-way，比7DyGFormer的3更难）")
...
log("=" * 80)
log(f"9DyGFormer_v9 [InfoNCE+chunked_neg] - Dataset: {args.dataset}")
log(f"  num_neg={args.num_neg}, p_pop={args.p_pop}, max_seq_len={args.max_seq_len}")
log(f"  neg_chunk={args.neg_chunk}, use_all_train={args.use_all_train}")
log("=" * 80)
...
# 验证集 DataLoader（只用 1 个随机负样本，标准 pairwise MRR）
...
if args.model_path:
    ...
    best_path = f"{save_path}/{args.dataset}_DyGFormerV9_best.pkl"
    last_path = f"{save_path}/{args.dataset}_DyGFormerV9.pkl"
