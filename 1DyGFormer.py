...
def test_competition(
    ...
    if skip_inference:
        try:
            ...
            if (
                ...
                print(
                    f"[r{RANK}] reusing cached shard ({my_n} queries), skipping inference",
                    flush=True,
                )
            else:
    ...
    print(
        f"[r{RANK}] shards written: {scores_path} ({os.path.getsize(scores_path)} bytes)",
        flush=True,
    )
    ...
    if missing:
        ...
        print(
            f"[r{RANK}] missing sentinels for ranks {missing}; leaving all shards in place. Rerun --eval_only to retry.",
            flush=True,
        )
        return None
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="Explicit model checkpoint path to load in eval_only mode (overrides the default best/latest auto-search)",
)
...
if IS_MAIN:
    if scores is None:
        log(
            "Aggregation skipped (not all ranks produced shards). Re-run --eval_only to retry; existing shards in save_dir will be reused."
        )
        sys.exit(0)
