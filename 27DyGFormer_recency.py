...
class DyGFormerRecency(DyGFormer):
    ...
    Recency features:
      1. exp(-delta_t / tau)              where tau = 30 days
      2. 1.0 / log1p(delta_t / 3600)      delta_t in seconds

    The features are computed on-the-fly inside get_features() so that every
    historical neighbor receives a time-decay encoding relative to the query
    interaction time.  Padding positions (neighbor_id == 0) are zero-masked.
    """

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Update edge_feat_dim to account for the 2 recency dimensions appended in get_features.
        # This ensures get_patches() reshapes to the correct size.
        self.edge_feat_dim += 2
        # Re-create the edge projection layer with the updated dimension.
    ...
    def get_features(self, node_interact_times: np.ndarray, padded_nodes_neighbor_ids: np.ndarray,
                     ...
                     time_encoder):
        """
        Override get_features to append 2-D recency features to edge_raw_features.
        """
        # 1. Obtain base features from the parent implementation.
        #    edge_feats shape: (batch_size, max_seq_length, edge_feat_dim)
        ...
        # 2. Compute recency features.
        #    delta_t in seconds; clamp to a small positive value to avoid division by zero.
        ...
        # 4. Concatenate recency to the original edge features.
        recency_var = jt.array(recency)
        edge_feats = jt.concat([edge_feats, recency_var], dim=-1)
        # Now edge_feats shape: (batch_size, max_seq_length, edge_feat_dim + 2)
...
# =============================================================================
# 3. Validation (16-way MRR for fast evaluation)
# =============================================================================
...
# Base edge_feat_dim = 128; DyGFormerRecency appends 2 recency dims internally.
# We keep edge_raw_features at 128 and let the subclass expand at runtime.
...
log("=" * 80)
log(f"DyGFormer + Recency Features (dataset1) — {DATE_STR}")
