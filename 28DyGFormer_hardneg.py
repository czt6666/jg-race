...
class MultiSourceHardNegativeSampler:
    ...
    If any source does not yield enough unique candidates after deduplication,
    the deficit is filled with random nodes so that every positive gets
    exactly 99 negatives (100-way total).

    Warm-up: first `warmup_epochs` use pure random negatives.
    """
    ...
    def _sample_one(self, s, d):
        """Generate exactly self.num_neg negatives for a single (src, dst) pair."""
        used = {int(d)}  # exclude the positive dst
        candidates = []

        # ---- 1) Historical neighbors (up to 33) ----
        hist = self.src_neighbors.get(s, set()) - used
        if hist:
            # Sort by interaction frequency (descending) and pick top-33
        ...
        # ---- 3) Popular nodes (up to 16) ----
        if len(self.top100_popular) > 0:
            # Shuffle top-100 to avoid always picking the same subset
        ...
        # Final safety fallback (should rarely trigger)
    ...
    def sample(self, src, dst, K=None):
        ...
        :param K: int, negatives per positive. Defaults to self.num_neg (99).
        ...
        # Warm-up: pure uniform random negatives
...
def compute_ranking_loss(pos_logit, neg_logit, temperature=1.0):
    ...
    With 99 negatives + 1 positive = 100-way ranking.
    """
    B = pos_logit.shape[0]
...
def test_val(model, loader, hard_sampler, val_num_neg=99):
    ...
    for _, batch_data in enumerate(loader_tqdm):
        ...
        # Hard negatives for validation (same 100-way setup)
...
def train(
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch_data in enumerate(train_tqdm):
            ...
            # Hard negatives (99 negatives -> 100-way)
            ...
            # v3: sync parameters across GPUs after every step
        ...
        # Gather training loss from all ranks for accurate logging
...
# =============================================================================
# 5. Test Inference (100-way MRR evaluation on 100 candidates)
# =============================================================================
...
def test_competition(model, test_src, test_time, test_candidates, tmp_dir, dataset_name, batch_size=50):
    ...
    # Reuse cached shards if present
...
# Fixed hyper-parameters for dataset1 (original DyGFormer)
...
# 100-way hard negative sampling
...
log("=" * 80)
log(f"DyGFormer + Multi-Source Hard Negatives (dataset1) — {DATE_STR}")
...
# Build multi-source hard negative sampler
...
# NOTE: negatives are generated manually per batch (100-way)
