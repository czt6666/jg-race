"""
11SASRec_highK.py - SASRec高K版本（K=31，32-way训练更接近100-way测试）

改进（相比 8SASRec）:
1. K=31 默认（32-way ranking训练 vs 测试100-way，比K=15更接近）
   - 理论: InfoNCE with K=99 = 直接优化100-way MRR
   - 实践: K=31 在速度和效果间取得平衡
2. 向量化 PopNegSampler (BatchChoice替代逐样本循环)
   - 一次性采样 B×K×OV 个候选再过滤
   - 速度提升约2倍（K大时效果更显著）
"""
import os
...
class PopNegSampler:
    ...
    def sample(self, src, dst, K):
        """向量化采样：一次性采 B×K×OV 个候选再过滤，比逐样本循环快约2倍"""
        B = len(src)
        ...
        ).astype(np.int32).reshape(B, n_pop * OV)
        uni_flat = self._rng.randint(
            self.min_dst, self.max_dst + 1, size=B * max(n_uni * OV, OV)
        ).astype(np.int32).reshape(B, -1)

        dst_col = np.array(dst, dtype=np.int32).reshape(B, 1)
        pop_mask = (pop_flat != dst_col)
        uni_mask = (uni_flat != dst_col)
        ...
        for i in range(B):
            ...
            if n_uni > 0:
                ...
                row = p_arr
            if len(row) < K:
                extra = self._rng.randint(
                    self.min_dst, self.max_dst + 1, size=K).astype(np.int32)
...
log("=" * 80)
log(f"11SASRec_highK [InfoNCE K=31 vectorized] - Dataset: {args.dataset}")
...
if IS_MAIN:
    ...
    output_file = f"{args.output_dir}/{args.dataset}/{args.dataset}_result_sasrec_highk.csv"
