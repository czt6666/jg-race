...
parser.add_argument("--num_neg", type=int, default=99, help="100-way: 1 pos + 99 neg")
...
log("=" * 80)
log(f"DyGFormer — 100-way AP, random negatives (dataset1) — {DATE_STR}")
log(f"Dataset: {args.dataset} | batch: {args.batch_size} | lr: {effective_lr}")
log(f"num_neg(train): {args.num_neg} | val_num_neg: {args.val_num_neg} | select_by: AP")
log("=" * 80)
...
if not args.eval_only:
    log(f"\nTraining — 100-way AP, random negatives, best selected by AP...")
    train(
        ...
        ranking_temperature=args.ranking_temperature, select_metric="AP",
    )
    mpi_barrier()
else:
