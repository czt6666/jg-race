...
# =============================================================================
# 0b. Repeat-RATIO Edge Features (NEW, replaces 40's raw-count scheme) — 8 cols.
#
#     Motivation: raw count(A,B) conflates very different relationships. "A sent
#     20 edges, 15 to B" (B = 75% of A's activity) and "A sent 1000 edges, 15 to
#     B" (1.5%) both give count=15. The right signal is the RATIO, normalized by
#     BOTH endpoints' degrees, plus the REVERSE direction (reciprocity / 双向边).
#
#     Per edge (u,v,t), strictly using history < t (single forward pass):
#       0 log1p(cnt_uv)        raw directed count (magnitude reference)
#       1 ratio_out = cnt_uv/out_total[u]   ← share of u's OUT activity toward v (core)
#       2 ratio_in  = cnt_uv/in_total[v]    ← share of v's IN activity from u
#       3 seen_before(u->v)    binary
#       4 log1p(cnt_vu)        REVERSE count v->u (bidirectional edge)
#       5 recip_out = cnt_vu/out_total[v]   share of v's OUT toward u (reciprocity)
#       6 is_reciprocal        binary (has v ever sent to u)
#       7 log1p(out_total[u])  u's overall activity level (the ratio denominator scale)
# =============================================================================

N_REPEAT_FEATS = 8

def add_repeat_ratio_features(src_ids, dst_ids, timestamps, original_edge_feat):
    """比例化 + 双向(互惠)的重复边特征。严格使用当前时刻之前的历史，防止信息泄漏。"""
    out_total = defaultdict(int)   # node -> #times seen as src
    in_total  = defaultdict(int)   # node -> #times seen as dst
    pair_cnt  = defaultdict(int)   # (u,v) -> directed count

    new_feats = []
    for u, v in zip(src_ids, dst_ids):
        cnt_uv = pair_cnt[(u, v)]
        cnt_vu = pair_cnt[(v, u)]
        out_u  = out_total[u]
        in_v   = in_total[v]
        ...
        ratio_out = cnt_uv / out_u if out_u > 0 else 0.0
        ratio_in  = cnt_uv / in_v  if in_v  > 0 else 0.0
        recip_out = cnt_vu / out_v if out_v > 0 else 0.0

        new_feats.append([
            np.log1p(cnt_uv),
            ratio_out,
            ratio_in,
            1.0 if cnt_uv > 0 else 0.0,
            np.log1p(cnt_vu),
            recip_out,
            1.0 if cnt_vu > 0 else 0.0,
            np.log1p(out_u),
        ])
        # update AFTER computing -> strict "history before this edge"
        pair_cnt[(u, v)] += 1
        out_total[u]     += 1
        in_total[v]      += 1

    extra = np.array(new_feats, dtype=np.float32)
    if original_edge_feat is None or original_edge_feat.shape[1] == 0:
        return extra
    return np.concatenate([original_edge_feat, extra], axis=1)


# =============================================================================
# 0c. DyGFormer with Recency Features (from 27/31), stacked on the repeat edge
#     features. edge_raw_features already carry 136 dims (128 base + 8 repeat-ratio);
#     this subclass appends 2 time-decay dims at runtime -> final edge_feat_dim=138.
# =============================================================================
...
# =============================================================================
# 0d. Random Negative Sampler — pure uniform random (NO hard negatives).
#     The 37/38 experiment proved hard negatives are a -0.08 regression on the
#     real test set (test negatives ~ random, not hard); see memory. 40 trains
#     and validates on uniform-random 100-way negatives to match test.
# =============================================================================
...
class RandomNegativeSampler:
    ...
    def reset_rng(self):
        # re-seed so the validation negatives are IDENTICAL every epoch -> stable yardstick
        self._rng = np.random.RandomState(self.seed)
...
def train(
    ...
    for epoch in range(num_epochs):
        ...
        log(f"Epoch {epoch + 1}: uniform-random {num_neg}-way negatives | loss={loss_type}")
        ...
        log(f"Epoch {epoch + 1}, random-neg Val: {val_res}")
...
ALGO_NAME        = "DyGFormer_repeat_ratio_bidir"
...
# 128 base + 8 repeat-ratio features; DyGFormerRecency appends +2 recency at runtime -> total 138
parser.add_argument("--edge_feat_dim",    type=int,   default=136)
...
parser.add_argument("--lr",               type=float, default=1e-4,
                    help="From-scratch lr (edge dim 128->134 means no warm-start possible).")
...
log("=" * 80)
log(f"DyGFormer — REPEAT-RATIO(bidir) + RECENCY + BIDIR-node, RANDOM 100-way negatives — {DATE_STR}")
log(f"Dataset: {args.dataset} | batch: {args.batch_size} | lr: {effective_lr} | epochs: {args.epochs}")
log(f"loss: {args.loss} | from scratch (no warm-start) | select by random-neg MRR")
log(f"Node feat: 96-out + 32-in bidir | Edge feat: 128 + 8 repeat-RATIO(bidir/recip) -> +2 recency = 138")
log("=" * 80)
...
# Edge features: 128-dim zero base + 4 repeat-edge stats (leakage-safe). The
# DyGFormerRecency backbone appends 2 recency dims at runtime -> 134 total.
log(f"Building repeat-RATIO edge features ({N_REPEAT_FEATS} leakage-safe cols: ratio_out/in + reciprocity)...")
base_edge_feat     = np.zeros((num_edges, args.edge_feat_dim - N_REPEAT_FEATS), dtype=np.float32)
enhanced_edge_feat = add_repeat_ratio_features(src_np, dst_np, t_np, base_edge_feat)
edge_raw_features  = np.zeros((num_edges + 1, args.edge_feat_dim), dtype=np.float32)
edge_raw_features[1:, :] = enhanced_edge_feat   # index 0 reserved for padding edge
log(f"Node features: {node_raw_features.shape}, edge features: {edge_raw_features.shape}\n")

# Uniform-random 100-way negatives for BOTH train and val. dst id range from the
# training graph. val sampler has its own seed + reset_rng -> stable yardstick.
dst_lo, dst_hi = int(dst_np.min()), int(dst_np.max())
train_sampler = RandomNegativeSampler(min_dst=dst_lo, max_dst=dst_hi, seed=42)
val_sampler   = RandomNegativeSampler(min_dst=dst_lo, max_dst=dst_hi, seed=123)
log(f"Samplers ready: uniform-random negatives, dst in [{dst_lo}, {dst_hi}].")
...
if not args.eval_only:
    # Train from scratch: edge_feat_dim changed 128 -> 134 (repeat+recency), so 36's
    # warm-start weights are dimensionally incompatible. No warm-start.
    log(f"\nTraining from scratch — random 100-way negatives, best selected by MRR...")
    train(
        model, optimizer, train_sampler, val_sampler, train_loader, val_loader,
