...
def train(backbone, optimizer, train_loader, val_loader, neg_sampler, K,
          ...
          early_stop_patience=5, lr=1e-3, lr_min=1e-5):
    """lr_min: cosine annealing 终止 LR，防止后期 epoch 过拟合"""
    best_mrr = 0.0
    patience_counter = 0

    for epoch in range(num_epochs):
        # Cosine annealing LR schedule
        cos_lr = lr_min + 0.5 * (lr - lr_min) * (1 + np.cos(np.pi * epoch / max(num_epochs, 1)))
...
log("=" * 80)
log(f"12SASRec_lrdecay [InfoNCE+CosineAnnealingLR] - Dataset: {args.dataset}")
...
if IS_MAIN:
    ...
    output_file = f"{args.output_dir}/{args.dataset}/{args.dataset}_result_sasrec_lrdecay.csv"
