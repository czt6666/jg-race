"""
109_SASRec_coldneighbor.py — SASRec + 冷启动邻居嵌入（Warm-up Cold Start）

算法原理：
  - 普通 SASRec 对冷启动物品的处理：随机初始化 embedding，测试时得分接近 0.5
  - 本方法：推理时，对每行候选集中的冷启动物品，
    用同一行已知物品嵌入的加权平均作为其临时表示
  - 权重策略：ID 距离越近的已知物品权重越高（ID proximity smoothing）
  - 这利用了"候选集里的已知物品可能与冷启动物品属于相同类别"的假设

关键区别：
  - 训练阶段：正常 SASRec 训练
  - 推理阶段：冷启动物品 embedding = 同候选集已知物品 embedding 的 ID-近邻加权平均

输出目录：109_SASRec_coldneighbor_MMDD
"""
import os
...
def get_cold_warm_emb(backbone, cands_row, max_train_item_id):
    """
    对一行 100 个候选物品，对冷启动物品用邻近已知物品嵌入平均来替换
    cands_row: [100] int array
    返回: [100, H] numpy array
    """
    H = backbone.item_embedding.weight.shape[-1]
    cand_ids = cands_row.astype(np.int32)
    is_cold = cand_ids > max_train_item_id

    with jt.no_grad():
        all_embs = backbone.item_embedding(jt.Var(cand_ids)).numpy()  # [100, H]

    if not is_cold.any():
        return all_embs

    # 已知物品的嵌入（用来初始化冷启动）
    warm_ids = cand_ids[~is_cold]
    warm_embs = all_embs[~is_cold]  # [n_warm, H]

    if len(warm_ids) == 0:
        return all_embs  # 全部冷启动，无法借用

    # 对冷启动物品：用 ID 最近的已知物品嵌入的加权平均
    cold_indices = np.where(is_cold)[0]
    for idx in cold_indices:
        cold_id = int(cand_ids[idx])
        # ID 距离权重（越近权重越高）
        dists = np.abs(warm_ids.astype(np.float32) - cold_id)
        sigma = max(dists.mean(), 1.0)  # 自适应带宽
        weights = np.exp(-dists / sigma)
        weights /= weights.sum() + 1e-8
        all_embs[idx] = (weights[:, np.newaxis] * warm_embs).sum(axis=0)

    return all_embs


def test_competition_coldwarm(backbone, test_src, test_time, test_cands, user_history, seq_len,
                               max_train_item_id, batch_size=32, cand_chunk=100):
    """使用冷启动邻居嵌入进行测试打分"""
    backbone.eval()
    N, K = test_cands.shape
    all_scores = np.zeros((N, K), dtype=np.float32)

    # 逐行处理（每行可能需要不同的冷启动初始化）
    for i in tqdm(range(0, N, batch_size), ncols=100, desc="Testing"):
        end = min(i + batch_size, N)
        src = test_src[i:end]
        t = test_time[i:end]
        cands = test_cands[i:end]  # [b, 100]
        b = len(src)

        item_seq, item_seq_len = get_batch_seqs(src, t, seq_len, user_history)
        with jt.no_grad():
            user_repr = backbone.forward(
                jt.Var(item_seq).int32(), jt.Var(item_seq_len).int32())  # [b, H]
        user_repr_np = user_repr.numpy()  # [b, H]

        for bi in range(b):
            # 为这行的候选物品计算嵌入（冷启动用邻居平均）
            cand_embs = get_cold_warm_emb(backbone, cands[bi], max_train_item_id)  # [100, H]
            # 打分
            row_scores = (user_repr_np[bi:bi+1, :] * cand_embs).sum(axis=-1)  # [100]
            all_scores[i + bi] = 1.0 / (1.0 + np.exp(-row_scores))  # sigmoid
...
log(f"=== {script_name} ===")
log(f"Dataset: {args.dataset}, cold-start neighbor embedding")
...
max_train_item_id = int(dst_all.max())
max_item_id = max(max_train_item_id, int(test_cands.max()))
log(f"Max train item: {max_train_item_id}, Max total: {max_item_id}")
log(f"Cold start items: IDs > {max_train_item_id}")
...
for epoch in range(args.epochs):
    ...
    if val_data is not None:
        ...
        if mrr > best_mrr:
            best_mrr = mrr
            jt.save(backbone.state_dict(), osp.join(save_dir, f"{args.dataset}_coldwarm_best.pkl"))
            log(f"  -> New best: {best_mrr:.4f}")
    else:
        jt.save(backbone.state_dict(), osp.join(save_dir, f"{args.dataset}_coldwarm_ep{epoch+1}.pkl"))
    jt.gc()

log("\nGenerating predictions with cold-start neighbor embedding...")
best_ckpt = osp.join(save_dir, f"{args.dataset}_coldwarm_best.pkl")
last_ckpt = osp.join(save_dir, f"{args.dataset}_coldwarm_ep{args.epochs}.pkl")
if osp.exists(best_ckpt):
    backbone.load_state_dict(jt.load(best_ckpt))
elif osp.exists(last_ckpt):
    backbone.load_state_dict(jt.load(last_ckpt))

scores = test_competition_coldwarm(
    backbone, test_src, test_time, test_cands, user_history, args.seq_len, max_train_item_id)
log(f"Scores: {scores.shape}, range=[{scores.min():.4f}, {scores.max():.4f}]")

out_file = osp.join(args.output_dir, args.dataset, f"{args.dataset}_result_coldwarm.csv")
