...
# =============================================================================
# 0b. Repeat Edge Features (from 26/31) — 4 leakage-safe columns appended to the
#     edge raw features: log(total past (u,v) co-occurrences), log(count in last
#     7d / 30d), and a binary "seen-before" flag. Strictly uses history < t.
# =============================================================================

def add_repeat_edge_features(src_ids, dst_ids, timestamps, original_edge_feat):
    """在原始边特征后追加4维重复边统计特征，严格使用当前时刻之前的历史，防止信息泄漏。"""
    pair_total = defaultdict(int)
    pair_times = defaultdict(list)

    new_feats = []
    for u, v, t in zip(src_ids, dst_ids, timestamps):
        key = (u, v)
        total = pair_total[key]
        history_times = pair_times[key]
        cnt_7d  = sum(1 for ht in history_times if t - ht <= 7  * 86400)
        cnt_30d = sum(1 for ht in history_times if t - ht <= 30 * 86400)
        new_feats.append([
            np.log1p(total),
            np.log1p(cnt_7d),
            np.log1p(cnt_30d),
...
#     features. edge_raw_features already carry 132 dims (128 base + 4 repeat);
#     this subclass appends 2 time-decay dims at runtime -> final edge_feat_dim=134.
# =============================================================================
...
ALGO_NAME        = "DyGFormer_repeat_recency_bidir"
...
# 128 base + 4 repeat features; DyGFormerRecency appends +2 recency at runtime -> total 134
parser.add_argument("--edge_feat_dim",    type=int,   default=132)
...
log("=" * 80)
log(f"DyGFormer — REPEAT + RECENCY + BIDIR features, RANDOM 100-way negatives — {DATE_STR}")
...
log(f"Node feat: 96-out + 32-in bidir | Edge feat: 128+4 repeat -> +2 recency = 134")
log("=" * 80)
...
log("Building repeat-edge features (4 leakage-safe columns)...")
