...
jt.flags.use_cuda = 1
# GPU1 上 K-RagMrc(7B LLM) 已占 ~13.5GB，Jittor 默认贪婪预分配全部显存会撞死它。
# 通过环境变量限制本进程显存（默认 9GB），与共卡任务安全共存。
_mem_gb = float(os.environ.get("JT_MEM_GB", "9"))
jt.flags.device_mem_limit = int(_mem_gb * 1024 ** 3)


# ============================================================
# GRU4Rec 模型（RNN 序列推荐，与 SASRec 注意力架构互补）
#   接口与 SASRec 对齐：
#     - item_embedding(ids) -> [N, H]
#     - forward(item_seq[B,L], item_seq_len[B]) -> [B, H] 用户表示
#   取最后一个有效位置(item_seq_len-1)的 GRU 隐状态作为用户表示。
# ============================================================
class GRU4Rec(nn.Module):
    def __init__(self, n_items, hidden_size=128, n_layers=2,
                 dropout=0.1, max_seq_length=64, initializer_range=0.02):
        super().__init__()
        self.n_items = n_items
        self.hidden_size = hidden_size
        self.initializer_range = initializer_range
        # 0 作为 padding，嵌入表覆盖所有 item（含冷启动）
    ...
    def _init_weights(self):
        ir = self.initializer_range
        self.item_embedding.weight.assign(
            jt.init.gauss(self.item_embedding.weight.shape, "float32", 0.0, ir))

    def execute(self, item_seq, item_seq_len):
        # item_seq [B, L], item_seq_len [B]（最小为1）
        emb = self.item_embedding(item_seq)          # [B, L, H]
        emb = self.emb_dropout(emb)
        gru_out, _ = self.gru(emb)                    # [B, L, H]
        B = item_seq.shape[0]
        # 取最后一个有效位置 (len-1) 的隐状态
        idx = (item_seq_len - 1).reshape(B, 1, 1)
        idx = idx.expand(B, 1, self.hidden_size)
        out = jt.gather(gru_out, 1, idx).squeeze(1)   # [B, H]
        out = self.out_dropout(out)
        return out

    # 训练/验证/测试代码统一调用 .forward()
    def forward(self, item_seq, item_seq_len):
        return self.execute(item_seq, item_seq_len)
IS_MPI = jt.in_mpi
...
def train(backbone, optimizer, train_loader, val_loader, neg_sampler, K,
    ...
    for epoch in range(num_epochs):
        ...
        if val_loader is not None:
            ...
            if cur > best_mrr:
                ...
                if IS_MAIN:
                    jt.save(backbone.state_dict(),
                            f"{save_path}/{dataset_name}_GRU4Rec_best.pkl")
                    os.sync()
            ...
            if IS_MAIN:
                jt.save(backbone.state_dict(),
                        f"{save_path}/{dataset_name}_GRU4Rec.pkl")
                os.sync()
            ...
            if IS_MAIN:
                ...
                ckpt = f"{save_path}/{dataset_name}_GRU4Rec_epoch{epoch+1}.pkl"
...
log("=" * 80)
log(f"20GRU4Rec [InfoNCE+WeightDecay+RNN] - Dataset: {args.dataset}")
...
log(f"GRU4Rec params: {sum(p.numel() for p in backbone.parameters()):,}")
...
if args.model_path:
    ...
    best_path = f"{save_path}/{args.dataset}_GRU4Rec_best.pkl"
    last_path = f"{save_path}/{args.dataset}_GRU4Rec.pkl"
...
if IS_MAIN:
    ...
    output_file = f"{args.output_dir}/{args.dataset}/{args.dataset}_result_gru4rec.csv"
